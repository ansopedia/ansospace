...
interface AuthFlowLoadingStateProps {
  title?: string;
  description?: string;
}

export function AuthFlowLoadingState({
  title = "Preparing Verification",
  description = "Please wait while we set things up for you...",
}: AuthFlowLoadingStateProps) {
  return (
    <AuthLayout>
      <div className="flex flex-col items-center justify-center space-y-6 py-12 text-center">
        ...
        <div className="space-y-3">
          ...
          <Typography className="text-muted-foreground text-sm">{description}</Typography>
        </div>
      </div>
    </AuthLayout>
  );
